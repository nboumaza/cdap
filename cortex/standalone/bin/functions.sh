    ...
    if [[ -z ${HIVE_HOME} ]] || [[ -z ${HIVE_CONF_DIR} ]] || [[ -z ${HADOOP_CONF_DIR} ]] || [[ -z ${HIVE_EXEC_ENGINE} ]] || [[ -z ${HIVE_CLASSPATH} ]]; then
      ...
      if [[ $(which ${HIVE_CMD} 2>/dev/null) ]]; then
        ...
        if [ ${__ret} -ne 0 ]; then
          ...
          echo "  If you do not want Hive functionality, set the 'explore.enabled' property in cdap-site.xml to 'false'"
...
cdap_service() {
  ...
  case ${__action} in
    status|stop|kill) cdap_${__action}_pidfile ${__pidfile} "${__name}"; __ret=${?} ;;
    start|restart|condrestart)
      if [[ ${__action} == condrestart ]]; then
        cdap_status_pidfile ${__pidfile} "${__name}" >/dev/null && \
          cdap_stop_pidfile ${__pidfile} "${__name}" && \
...
cdap_start_bin() {
  ...
  logecho "$(date) Starting ${__name} service on ${HOSTNAME}"
...
cdap_start_java() {
  ...
  if [[ ${CDAP_SERVICE} == master ]]; then
    ...
    if [[ ${__startup_checks} == true ]]; then
      logecho "$(date) Running Master startup checks -- this may take a few minutes"
...
cdap_sdk_usage() {
  ...
  echo "--enable-debug [ <port> ] to connect to a debug port for Standalone (default port is 5005)"
...
# cdap_sdk_stop
#
cdap_sdk_stop() { cdap_stop_pidfile ${__pidfile} "Standalone (SDK)"; };
...
cdap_sdk_start() {
  ...
  # Start SDK processes
  echo -n "$(date) Starting Standalone (SDK) ..."
  if ${__foreground}; then
    echo
...
cdap_auth() {
  ...
  cdap_start_java || die "Failed to start ${CDAP_SERVICE} service"
}
...
cdap_ui() {
  ...
  cdap_start_bin || die "Failed to start ${CDAP_SERVICE} service"
}
...
cdap_sdk() {
  ...
  case ${__action} in
    start|restart)
      ...
      __ret=${?}
      ;;
    status) cdap_status_pidfile ${__pidfile} "Standalone (SDK)"; __ret=${?} ;;
